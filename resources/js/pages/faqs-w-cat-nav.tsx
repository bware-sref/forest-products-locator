        ...
        <AppLayout>
            ...
            <div className="flex min-h-screen flex-col items-center bg-nature p-6 text-beluga lg:justify-center lg:p-8 dark:bg-nature">
                <div className="flex flex-col w-full lg:max-w-7xl items-center lg:items-start justify-start opacity-100 transition-opacity duration-750 lg:grow starting:opacity-0 px-6 gap-5">
                    ...
                    {/* wrapper for FAQ List and FAQ TOC */}
                    <div className="flex flex-row w-full bg-orange-400">

                        {/* I was originally thinking order-last but maybe we want to position it statically? */}
                        <div className="flex flex-col w-1/3 sticky top-0 right-0 order-last bg-purple-400">
                            <h2 className="font-bold text-2xl pb-4">Categories</h2>
                            <ul className="flex flex-col gap-4">
                                {faqsByCategory.map((category) => (
                                    <li
                                        ...
                                        className="pl-4"
                                    >
                                        <a href={`#${category.slug}`} className="underline hover:no-underline">
                                            {category.name}
                                        </a>
                                    </li>
                                ))}
                        ...
                        <ul className="flex flex-col gap-4  max-h-screen overflow-y-auto">
                            {faqsByCategory.map((category) => (
