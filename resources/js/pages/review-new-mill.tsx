...
export default function ReviewNewMill({ submitted, ...props }: MillDiffProps) {
        ...
        if (Array.isArray(value)) {
            ...
            return (
                <div className="flex flex-wrap gap-1.5">
                    {value.map((item, idx) => (
                        <span key={idx} className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs ring-1 bg-velvet font-medium Xtext-slate-300 text-beluga ring-slate-700/50'}`}>
    ...
    return (
        <AppLayout>
            <div className="min-h-screen bg-nature text-beluga p-6 md:p-10 font-sans selection:bg-emerald-500/30">
                {/* Header section */}
                <header className="mb-8">
                    <div className="flex items-center gap-3">
                        <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-white">New Mill Submission Review</h1>
                    </div>
                    <p className="mt-2 text-sm md:text-base text-beluga">
                        Review user-submitted Mill information.
                    </p>
                </header>
                                ...
                                <tbody className="divide-y divide-slate-800/60">
                                    ...
                                    {Object.entries(submitted).map(([key, value]) => {
                                        ...
                                        return (
                                            <tr 
                                                key={key} 
                                                className={`transition-colors duration-150 odd:bg-aircraft even:bg-coupe hover:bg-slate-800/40`}
                                            >
                                                {/* Field Name */}
                                                <td className="p-4 font-semibold antialiased flex items-center gap-2">
                                                    <span className="font-mono text-beluga">{key}</span>
                                                </td>
                                                {/* New Mill */}
                                                <td className={`p-4 font-mono Xtext-slate-300 text-beluga`}>
                                                    {renderValue(value)}
                                                </td>
                                            </tr>
                                        );
